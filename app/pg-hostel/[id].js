...
export default function PgHostelDetails() {
  ...
  const handleShare = async () => {
    try {
      const shareMessage = `Check out this PG/Hostel: ${item?.title}
📍 ${item?.location?.fullAddress}
💰 ₹${item?.priceRange?.min} - ₹${item?.priceRange?.max}
🏠 ${item?.availableSpace} beds available
...
Download the app to view more details!`;
      ...
      const result = await Share.share({
        ...
        title: item?.title || 'PG/Hostel Listing',
      });

      if (result.action === Share.sharedAction) {
        showToast('PG/Hostel shared successfully!');
      }
    } catch (error) {
      console.error('Error sharing:', error);
      showToast('Failed to share PG/Hostel');
    }
  };
  ...
  const handleReportSubmit = async () => {
    ...
    try {
      ...
      showToast('PG/Hostel reported successfully.');
      setIsReportModalVisible(false);
      setReportReason('');
      setReportDescription('');
    } catch (error) {
      console.error('Error reporting PG/Hostel:', error);
      const errorMessage = error.response?.data?.message || 'Failed to report PG/Hostel';
  ...
  const toggleFavorite = async () => {
    ...
    try {
      ...
      console.log('🔄 Toggling favorite for PG/Hostel:', id);
            ...
            <View style={styles.reportModalOverlay}>
              <View style={styles.reportModalContent}>
                <View style={styles.reportHeader}>
                  <Text style={styles.reportTitle}>Report PG/Hostel</Text>
